...
def fetch_token():
    ...
    if ('access_token' in result.keys() and 'scope' in result.keys()):
        if not config.SCOPE in result['scope'].split(' '):
...
def baidu_synthesis(TEXT,voice = config.VOL):
    # config = baidu_config()
    token = fetch_token()
    tex = quote_plus(TEXT)  # 此处TEXT需要两次urlencode
    # print(tex)
    params = {'tok': token, 'tex': tex, 'per': voice, 'spd': config.SPD, 'pit': config.PIT, 'vol': config.VOL, 'aue': config.AUE, 'cuid': config.CUID,
              'lan': 'zh', 'ctp': 1}  # lan ctp 固定参数
    # print(params)
    data = urlencode(params)
    # print('test on Web Browser' + TTS_URL + '?' + data)

    req = Request(config.TTS_URL, data.encode('utf-8'))
